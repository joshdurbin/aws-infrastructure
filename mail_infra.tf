resource "digitalocean_droplet" "mail_server" {
  ...
  ssh_keys = [
    ...
    "${digitalocean_ssh_key.ios_pub_key.id}"
  ]
...
resource "digitalocean_ssh_key" "platypus_pub_key" {
  name = "platypus"
  public_key = "${file("/Users/jdurbin/crypto/ssh/jdurbin_platypus.pub")}"
}

resource "digitalocean_ssh_key" "ios_pub_key" {
  name = "platypus"
  public_key = "${file("/Users/jdurbin/crypto/ssh/jdurbin_ios_mobile.pub")}"
}